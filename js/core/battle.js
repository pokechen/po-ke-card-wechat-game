...
function createMatch(options = {}) {
  ...
  const aiFaction = options.aiFaction || "Monsters";
  const difficulty = options.difficulty || "normal";
  const activeSlot = getActiveCustomDeckSlotIndex(options, humanFaction);
  const customIds = options.customDeckEnabled ? getActiveCustomDeckIds(options, humanFaction) : [];
  ...
  const aiLeaderId = options.aiLeaderIds?.[aiFaction] || options.aiLeaderId;
  const players = [
  ...
  recalcScores(state);
  addLog(state, `先换牌：每名玩家最多 ${state.mulligan.max} 张，可直接开始。`);
...
function isClearWeather(card) {
  return card.category === "weather" && (card.baseName === "拨云见日" || card.baseName === "Clear Weather");
}
...
function playCard(state, cardUid, preferredRow) {
  ...
  if (card.category === "weather" || card.category === "special") {
    ...
    const target = hasAbility(card, "Spy") ? otherIndex(player.index) : player.index;
    const row = preferredRow || rowForCard(state, player.index, card) || "melee";
    card.playedBy = player.index;
    card.owner = target;
...
function useLeader(state, playerIndex) {
  ...
  if (/cancel your opponent|取消.*主将|cancel.*leader/.test(text)) {
    ...
    doScorch(state, playerIndex, rowFromLeaderText(text), true);
...
function resolveSpecial(state, playerIndex, card, row) {
  ...
  if (isScorch(card)) return doScorch(state, playerIndex, null, false);
  if (isDecoy(card)) {
...
function resolveUnitAbility(state, playedBy, target, row, card) {
  ...
  if (isScorch(card) && card.category !== "special") doScorch(state, playedBy, row, true);
}
...
function doMuster(state, playedBy, target, row, card) {
  ...
  [player.hand, player.deck].forEach(pile => {
    for (let i = pile.length - 1; i >= 0; i--) {
      const item = pile[i];
      if (item.uid !== card.uid && names.includes(item.baseName)) {
        pile.splice(i, 1);
        const targetRow = (item.row || []).includes(row) ? row : ((item.row || [])[0] || row);
        item.playedBy = playedBy;
        item.owner = target;
        state.players[target].board[targetRow].push(item);
        moved += 1;
      }
    }
  });
...
function reviveCardByUid(state, playerIndex, uid) {
  ...
  addLog(state, `举荐生效：${player.name}复归「${cardLabel(card)}」。`);
  return true;
}
...
function doScorch(state, playerIndex, row, opponentOnly) {
  recalcScores(state);
  const targets = [];
  ...
  players.forEach(pi => {
    ROWS.forEach(r => {
      ...
      state.players[pi].board[r].forEach(card => {
        if (!card.hero && (card.effective || 0) >= 10) targets.push({ pi, row: r, card });
      });
    });
  });
  ...
  state.players.forEach(player => {
    player.board[row].forEach(card => {
      if (hasAbility(card, "Berserker") && !card.transformed) {
        ...
        card.name = bear?.baseName || "Transformed Bear";
    ...
    ROWS.forEach(row => {
      ...
      cards.forEach(card => {
        ...
        if (!card.hero && state.weather[row]) {
          value = player.leader && player.leader.baseName === "King Bran" ? Math.ceil(value / 2) : Math.min(value, 1);
        }
...
function estimatePlayGain(state, playerIndex, card) {
  ...
  if (isScorch(card)) return scorchGain(state, playerIndex);
...
function scorchGain(state, playerIndex) {
  recalcScores(state);
  const targets = [];
  [0, 1].forEach(pi => ROWS.forEach(row => state.players[pi].board[row].forEach(card => {
    if (!card.hero && (card.effective || 0) >= 10) targets.push({ pi, card });
  })));
