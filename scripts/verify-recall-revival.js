...
// 验证：在卡组包含「非传世济世」（张仲景，非 Hero 的 Revival）的前提下，
// 携带 0 / 1 / 2 / 3 张「请辞归隐」（Recall）时的胜率区别。
//
// 设计：
//  - 固定阵营为 开国群雄，使张仲景可入组。
//  - 每个种子先生成一份基准困难卡组（作为对手与受试卡组的来源）。
//  - 受试卡组 = 基准卡组，但确保恰好 1 张张仲景（zhangyu-0145，非传世济世），
...
const FACTION = "开国群雄";
const REVIVAL_CARD_ID = "zhangyu-0145"; // 张仲景：非传世济世
...
function main() {
  ...
  const lines = [];
  lines.push(`验证「非传世济世(张仲景) + N 张请辞归隐」胜率区别`);
